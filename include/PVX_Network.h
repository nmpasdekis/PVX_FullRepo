...
namespace PVX::Network {
	...
	public:
		...
		void Json(const PVX::JSON::Item & Json);
		void Run(const std::wstring & Function);
	...
	protected:
		...
		friend class WebSocket;
	public:
		enum class WebSocker_Opcode : uint8_t {
			Opcode_Continued = 0,
			Opcode_Text,
			Opcode_Bynary,
	...
	public:
		...
		std::vector<std::string> GetConnectionsGroups(const std::string& Name);
	...
	std::string MakeJavaWebToken(const std::vector<uint8_t>& secret, const std::initializer_list<std::pair<std::wstring, std::variant<std::wstring, int64_t>>>& data, int ExpireMinutes = -1);
	inline std::string MakeJavaWebToken(const std::string& secret, const std::initializer_list<std::pair<std::wstring, std::variant<std::wstring, int64_t>>>& data, int ExpireMinutes = -1) { return MakeJavaWebToken(PVX::Decode::Base64(secret), data, ExpireMinutes); }
	inline std::wstring MakeJavaWebTokenW(const std::vector<uint8_t>& secret, const std::initializer_list<std::pair<std::wstring, std::variant<std::wstring, int64_t>>>& data, int ExpireMinutes = -1) { return PVX::Encode::ToString(MakeJavaWebToken(secret, data, ExpireMinutes)); }
	inline std::wstring MakeJavaWebTokenW(const std::string& secret, const std::initializer_list<std::pair<std::wstring, std::variant<std::wstring, int64_t>>>& data, int ExpireMinutes = -1) { return PVX::Encode::ToString(MakeJavaWebToken(PVX::Decode::Base64(secret), data, ExpireMinutes)); }
	PVX::JSON::Item ValidateJavaWebToken(const std::vector<uint8_t>& secret, const std::string& token, bool expires = true);
	inline PVX::JSON::Item ValidateJavaWebToken(const std::string& secret, const std::string& token, bool expires=true) { return ValidateJavaWebToken(PVX::Decode::Base64(secret), token, expires); }
	inline PVX::JSON::Item ValidateJavaWebToken(const std::string& secret, const std::wstring& token, bool expires = true) { return ValidateJavaWebToken(PVX::Decode::Base64(secret), PVX::Encode::ToString(token), expires); }
	inline PVX::JSON::Item ValidateJavaWebToken(const std::vector<uint8_t>& secret, const std::wstring& token, bool expires = true) { return ValidateJavaWebToken(secret, PVX::Encode::ToString(token), expires); }
	...
	class SyncServer {
		using srvFunction = std::function<void(WebSocketServer& ws, const std::string&, const PVX::JSON::Item&)>;
		WebSocketServer& ws;
		std::unordered_map<std::wstring, srvFunction> Functions;
		friend WebSocketServer& CreateSyncServer(HttpServer& http, std::wstring url, SyncServer* server);
	public:
		SyncServer(HttpServer& http, std::wstring url);
		void OnConnect(std::function<void(const std::string&, WebSocket&)> fnc);
		void OnDisconnect(std::function<void(const std::string&)> fnc);
		void AddFunction(const std::wstring& Name, srvFunction ServerFunction);
	};
}
#endif