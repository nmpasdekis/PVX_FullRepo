...
namespace PVX {
	namespace OpenGL {
		...
		struct SimpleMatrial {
			...
			Vector4D Specular;
			Vector4D Emission;
			float SpecularPower, Transparency;
			struct {
				Texture2D Ambient;
				Texture2D Diffuse;
				Texture2D Specular;
			...
			inline void Bind(PVX::OpenGL::Context& gl) {
		...
		struct InterleavedObjectMaterial {
			SimpleMaterial Material;
			std::map<std::string, InterleavedArrayObject> Object;
		};
