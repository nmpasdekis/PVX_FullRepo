...
namespace PVX {
	namespace DB {
		...
		public:
			...
			const std::wstring & FieldName(int Index);
			const int FieldType(int Index);
			const int FieldType(const std::wstring & Name);
			Ref & operator[](int Index);
			Ref & operator[](const std::wstring & Name);
			std::vector<unsigned char> & operator()(int Index);
			std::vector<unsigned char> & operator()(const std::wstring & Name);
			int IsNull(int Index);
			int IsEmpty(int Index);
			int IsNullOrEmpty(int Index);
			int IsNull(const std::wstring & Name);
			int IsEmpty(const std::wstring & Name);
			int IsNullOrEmpty(const std::wstring & Name);
		...
		public:
			...
			PVX::JSON::Item JsonUpdateJson(const std::wstring& Table, const const PVX::JSON::Item & Values, const std::set<std::wstring> & ConditionParams, const std::wstring& Where = L"", const std::vector<std::wstring>& ReturnFields = {});
