...
#define HasTex3D 8
		...
		std::vector<PVX::Vector3D> InterleavedArrayObject::GetTriangles() const {
			auto verts = InterleavedArrayObject::GetTrianglesIndexed();
			std::vector<PVX::Vector3D> ret;
			ret.reserve(Index.size());
			for (auto i : Index) ret.push_back(verts[i]);
			return ret;
		}

		std::vector<PVX::Vector3D> InterleavedArrayObject::GetTrianglesIndexed() const {
			std::vector<PVX::Vector3D> ret(Data.size() / Stride);
			Interleave(ret.data(), sizeof(PVX::Vector3D), Data.data(), Stride, ret.size());
			return ret;
		}
		...
		InterleavedArrayObject MakeSquareWithUV_Zero() {
			PVX::OpenGL::ObjectBuilder gl;
			gl.Begin(GL_QUADS); {
				gl.TexCoord({ 0.0f, 0.0f }); gl.Vertex({ 0.0f, 0.0f, 0 });
				gl.TexCoord({ 1.0f, 0.0f }); gl.Vertex({ 1.0f, 0.0f, 0 });
				gl.TexCoord({ 1.0f, 1.0f }); gl.Vertex({ 1.0f, 1.0f, 0 });
				gl.TexCoord({ 0.0f, 1.0f }); gl.Vertex({ 0.0f, 1.0f, 0 });
			}gl.End();
			return gl.Build();
		}
